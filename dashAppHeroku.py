...
def retonarDf(EstadoAlvo, nomeMeses, ICMS, compilado):
    ...
    dataF = pd.DataFrame({label: [percentualToString(retornaSufNew(EstadoAlvo, nomeMeses, ICMS, compilado, IPVA))]},
                         index=[EstadoAlvo])
...
if alertaUpToDate[0] == False:
    ...
    if alertaUpToDate[1] > 1:
        textoAlerta = str(alertaUpToDate[1]) + " Estados ainda não atualizaram os dados de arrecadação para " + \
                      nomeMeses[-1]
    else:
    ...
    dbc.Row([
        ...
        dbc.Col(
            html.Div([
                html.Div([
                    dash_table.DataTable(
                        ...
                        columns=[{"name": i, "id": i} for i in
                                 retonarDf(beginner, nomeMeses, ICMS, compilado)[2].columns],
                ...
                html.Div([dbc.Alert(textoAlerta, color=corAlerta, style={'margin-top': '5%'},
                                    dismissable=not alertaUpToDate[0])]),
    ...
    dbc.Row(
        ...
        dbc.Col(
            ...
            dcc.Dropdown(
                ...
                value=dadosCapitais[-3][1],
                # setando o valor base como o total das capitais, o antepenúltim item da lista
                clearable=False), width={"size": 10, "offset": 1})
    ),
...
def update_output(value):
    ...
    graficoEstados.add_trace(go.Bar(x=eixos[0], y=eixos[1]
                                    , name='Suporte LC 173 - Repasse de Recursos + Suspensão de Dívida',
    ...
    graficoMesMes = go.Figure(
        ...
        data=[
            go.Bar(name="Arrecadação 2019", x=eixosMM[0], y=eixosMM[1], offsetgroup=0, marker_color='rgb(55, 83, 109)',
                   text=labelsBarra1, textposition='outside'),
            go.Bar(name="Arrecadação 2020 + Suporte LC 173", x=eixosMM[0], y=tempEmpilhado, offsetgroup=1,
                   marker_color='rgb(229, 165, 17)', text=labelsBarra2, textposition='outside'),
            go.Bar(name="Arrecadação 2020", x=eixosMM[0], y=eixosMM[2], offsetgroup=1, marker_color='rgb(110, 80, 10)')
        ],
        layout=go.Layout(title='Arrecadação (ICMS + IPVA) e Suporte (LC 173) - Mês a Mês', yaxis_title='em R$',
                         legend_orientation="h")
        # fim graficoMesMes
    )
